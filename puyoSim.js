...
window.resetGame = function() { 
    clearInterval(dropTimer); // タイマーを停止
    initializeGame();
    // alert('盤面をリセットしました。'); // デバッグ用なのでコメントアウト
}
...
function handleInput(event) {
    ...
    switch (event.key) {
        ...
        case 'X':
            ...
            break;
        case 'ArrowDown':
            // 自動落下タイマーを一時的にリセットし、加速
            clearInterval(dropTimer);
            movePuyo(0, -1); 
            startPuyoDropLoop(); // タイマーを再開
            break;
        case ' ': 
